...
class Program
{
    ...
    static void TestLinq(string path)
    {
        var baseDirectory = AppDomain.CurrentDomain.BaseDirectory;
        var absolutePath = Path.Combine(baseDirectory, path);

        TheList list = GetFileList(absolutePath);
        ...
        string[] categories = new string[] { "Work", "University", "Personal", "Family" };

        foreach (string cat in categories)
        {
            Console.Write($"     number of tasks in \"{cat}\" category: ");
            Console.WriteLine(list.Tasks.Where(t => t.Category.Equals(cat))
